...
export function Dialog(props: Props): ReactElement {
  ...
  useEffect(() => {
    widgetMgr.setFormState(formId, clearOnSubmit, closeOnSubmit, setIsOpen)
  }, [widgetMgr, formId, clearOnSubmit, closeOnSubmit, setIsOpen])

  if (!isOpen && shouldDialogBeOpened(formId)) {
    clearShouldDialogBeOpened(formId)
    setIsOpen(true)
    clearDialogParams(formId)
  } else if (isOpen && shouldDialogBeClosed(formId)) {
    closeDialog(formId)
    setIsOpen(false)
    clearDialogParams(formId)
  }
